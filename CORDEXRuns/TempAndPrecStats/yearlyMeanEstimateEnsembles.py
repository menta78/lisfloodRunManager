...
def loopModels(inputDir, inputFlName, ncVarNames, outputDir):
  ...
  for mdl in models:
    ...
    outR8FlPath = os.path.join(outputDir, 'mean_' + ncVarNames[0] + '_rcp85_' + mdl + '_atWarmingLev.nc')
    outR4FlPath = os.path.join(outputDir, 'mean_' + ncVarNames[0] + '_rcp45_' + mdl + '_atWarmingLev.nc')

    print('elaborating file ' + outR8FlPath)
    extract1Model('rcp85', mdl, histFlPath, r8FlPath, outR8FlPath, varnames=ncVarNames)
    print('elaborating file ' + outR4FlPath)
    extract1Model('rcp45', mdl, histFlPath, r4FlPath, outR4FlPath, varnames=ncVarNames)


def computeEnsemble(flsDir, filePrefix):
  outFlName = filePrefix + '_yearly_ensemble.nc'
  outFlPath = os.path.join(flsDir, outFlName)
  fls = [f for f in os.listdir(flsDir) if (f != outFlName) and (re.match(filePrefix, f))]
  nfls = len(fls)
  outDlt = None
  outAbsDlt = None
...
if __name__ == '__main__':
 ...
 # precipitations
  inputDir = '/DATA/ClimateData/cordexEurope/yearlymeans/rawData/'
  inputFlName = 'pr.nc'
  outputDir = '/DATA/ClimateData/cordexEurope/yearlymeans/atWarmingLev/'
 ...
 # temperatures
  ...
  loopModels(inputDir, inputFlName, ncVarNames, outputDir)
  computeEnsemble(outputDir, filePrefix)
