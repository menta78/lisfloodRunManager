function lfEvaModelHelix(scenario, model, outDir, varargin)
...
retLevNcFlName = [strjoin({'projection', 'HELIX', varname, scenario, model, 'statistics'}, '_') '.nc'];
...
%     for isuby = 2:2
      ...
      tic;
      [ tmstmp, xx_, yy_, vls ] = lfDisLoadFromNcHelix( [iLonStart iLatStart], [iLonEnd iLatEnd], model, varargin{:} );
     %evaData = lfExecuteTsEva( tmstmp, xx_, yy_, vls, outYears, returnPeriodsInYears, channelMap_, 'parObj', parObj );
      evaDataMax = lfExecuteTsEvaHelix( tmstmp, xx_, yy_, vls, outYears, returnPeriodsInYears, 'nWorker', nparworker, varargin{:} );
      ...
      evaDataMin = lfExecuteTsEvaGEVHelix( yydt, xx_, yy_, vlsmnmin_, outYears, returnPeriodsInYears, 'nWorker', nparworker, 'maxFracNan', .1, varargin{:} );
      ...
      elseif size(yMean, 3) < size(vlsYMean, 3)
        yMean(iLatStart:iLatEnd, iLonStart:iLonEnd, :) = vlsYMean(:,:,1:size(yMean, 3));
      else
  ...
  % max
  nccreate(retLevNcOutFilePath, 'rl', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout, 'return_period', nretper});
  ncwrite(retLevNcOutFilePath, 'rl', retLevGPD);

  nccreate(retLevNcOutFilePath, 'se_rl', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout, 'return_period', nretper});
  ncwrite(retLevNcOutFilePath, 'se_rl', retLevErrGPD);

  nccreate(retLevNcOutFilePath, 'shape_fit', 'dimensions', {'lat', ny, 'lon', nx});
  ncwrite(retLevNcOutFilePath, 'shape_fit', shapeGPD);

  nccreate(retLevNcOutFilePath, 'se_shape', 'dimensions', {'lat', ny, 'lon', nx});
  ncwrite(retLevNcOutFilePath, 'se_shape', shapeGPDErr);

  nccreate(retLevNcOutFilePath, 'scale_fit', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout});
  ncwrite(retLevNcOutFilePath, 'scale_fit', scaleGPD);

  nccreate(retLevNcOutFilePath, 'se_scale', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout});
  ncwrite(retLevNcOutFilePath, 'se_scale', scaleGPDErr);

  nccreate(retLevNcOutFilePath, 'threshold_fit', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout});
  ncwrite(retLevNcOutFilePath, 'threshold_fit', thresholdGPD);

  nccreate(retLevNcOutFilePath, 'se_threshold', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout});
  ncwrite(retLevNcOutFilePath, 'se_threshold', thresholdGPDErr);
  
  nccreate(retLevNcOutFilePath, 'year_max', 'dimensions', {'lat', ny, 'lon', nx, 'year_all', nyrall});
  ncwrite(retLevNcOutFilePath, 'year_max', yMax);

  % min
  nccreate(retLevNcOutFilePath, 'rl_min', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout, 'return_period', nretper});
  ncwrite(retLevNcOutFilePath, 'rl_min', retLevGEVMin);

  nccreate(retLevNcOutFilePath, 'se_rl_min', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout, 'return_period', nretper});
  ncwrite(retLevNcOutFilePath, 'se_rl_min', retLevErrGEVMin);

  nccreate(retLevNcOutFilePath, 'shape_fit_min', 'dimensions', {'lat', ny, 'lon', nx});
  ncwrite(retLevNcOutFilePath, 'shape_fit_min', shapeGEVMin);

  nccreate(retLevNcOutFilePath, 'se_shape_min', 'dimensions', {'lat', ny, 'lon', nx});
  ncwrite(retLevNcOutFilePath, 'se_shape', shapeGEVErrMin);

  nccreate(retLevNcOutFilePath, 'scale_fit_min', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout});
  ncwrite(retLevNcOutFilePath, 'scale_fit_min', scaleGEVMin);

  nccreate(retLevNcOutFilePath, 'se_scale_min', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout});
  ncwrite(retLevNcOutFilePath, 'se_scale_min', scaleGEVErrMin);

  nccreate(retLevNcOutFilePath, 'location_fit_min', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout});
  ncwrite(retLevNcOutFilePath, 'location_fit_min', locationGEVMin);

  nccreate(retLevNcOutFilePath, 'se_location_min', 'dimensions', {'lat', ny, 'lon', nx, 'year', nyrout});
  ncwrite(retLevNcOutFilePath, 'se_location_min', locationGEVErrMin);
  
  nccreate(retLevNcOutFilePath, 'year_min', 'dimensions', {'lat', ny, 'lon', nx, 'year_all', nyrall});
  ncwrite(retLevNcOutFilePath, 'year_min', yMin);

  
  % mean
  nccreate(retLevNcOutFilePath, 'year_mean', 'dimensions', {'lat', ny, 'lon', nx, 'year_all', nyrall});
  ncwrite(retLevNcOutFilePath, 'year_mean', yMean);

  
  
  nccreate(retLevNcOutFilePath, 'lon', 'dimensions', {'ix', nx});
  ncwrite(retLevNcOutFilePath, 'lon', xx);

  nccreate(retLevNcOutFilePath, 'lat', 'dimensions', {'iy', ny});
  ncwrite(retLevNcOutFilePath, 'lat', yy);
