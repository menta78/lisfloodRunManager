...
def plotTimeSerie30YVariability_hiExt(ax=None, plotXLabel=True, warmingLev=2.0):
 ...
 #pmdlr4 = plt.plot(yr, r4spmean.transpose()*100, 'skyblue', label='RCP4.5 models ($\sigma={sgm:1.2f}\%, skw.={s:1.2f})$'.format(sgm=r4sigma*100, s=r4skew)); 
  ...
  pmedr8 = plt.plot(yr, r8median*100, 'firebrick', linewidth=6, label='RCP8.5 mean'); 
  pmedr4 = plt.plot(yr, r4median*100, 'royalblue', linewidth=6, label='RCP4.5 mean'); 
  plt.grid('on')
...
def plotTimeSerie30YVariability_lowExt(ax=None, plotXLabel=True, warmingLev=2.0):
...
def plotTimeSerie30YVariability_mean(ax=None, plotXLabel=True, warmingLev=2.0):
  _, _, _, r8srs, r4srs = loadWlVsScenChange.loadMeanChangesAtWl_nYearsAroundWLYear(warmingLev=warmingLev, nmodels=nmodels)
 ...
 #pmdlr4 = plt.plot(yr, r4spmean.transpose()*100, 'skyblue', label='RCP4.5 models ($\sigma={sgm:1.2f}\%, skw.={s:1.2f}$)'.format(sgm=r4sigma*100, s=r4skew)); 
  ...
  plt.legend(handles=[pmdlr8[0], pmedr8[0], pmdlr4[0], pmedr4[0]])
  if plotXLabel:
    plt.xlabel('years to $' + str(warmingLev) + '^\circ$ C w.l.', fontsize=14)
...
def plotTimeSerie30YVariability_hiLowExt(warmingLev=2.0):
  ...
  plotTimeSerie30YVariability_mean(ax1, plotXLabel=False, warmingLev=warmingLev)
  ylm = plt.ylim()
  ytxt = min(ylm) + (max(ylm) - min(ylm))*.93
  plt.text(xtxt, ytxt, 'b', fontsize=15)

  ax3 = plt.subplot(gs[2,0])
  plotTimeSerie30YVariability_lowExt(ax1, plotXLabel=True, warmingLev=warmingLev)
  ylm = plt.ylim()
  ytxt = min(ylm) + (max(ylm) - min(ylm))*.93
