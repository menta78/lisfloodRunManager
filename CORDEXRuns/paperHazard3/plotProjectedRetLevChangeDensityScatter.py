...
def plotSingleModel(ax, relChngR8, relChngR4, modelName, plotRegression=False, plotPPGraph=False, showLegend=False, densityBins=70, lim=None, **kwargs):
 ...
 #ax.yaxis.set_label_position('right')
  ...
  txty = (max(lim) - min(lim))/1.1 + min(lim)
...
def plotEnsembles_means(rootDir='/ClimateRun4/multi-hazard/eva/', gs=None, showLegend=True):
  warmingLevs = [1.5, 2.0]
  rcp8, rcp4 = 'rcp85', 'rcp45'
...
# lims = {1.5: [-24, 60], 2.0: [-39, 80]}
  ...
  for warmingLev, iwl in zip(warmingLevs, range(len(warmingLevs))):
    ...
    for mdl, imdl in zip(models, range(1, len(models) + 1)):
      ...
      wrmYear = wlYearR8[mdl]
      wrmYear = int(round(wrmYear/5.)*5.)
      relChngR8, vlsMask = getRelChngs(ncFlPth, None, wrmYear, ncvar='year_mean', vlsMask=vlsMask)
  
      ncFlNm = '_'.join(['projection_dis', rcp4, mdl, wuStr, 'statistics.nc'])
      ncFlPth = os.path.join(rootDir, ncFlNm)
      wrmYear = wlYearR4[mdl]
      wrmYear = int(round(wrmYear/5.)*5.)
      relChngR4, vlsMask = getRelChngs(ncFlPth, None, wrmYear, ncvar='year_mean', vlsMask=vlsMask)
